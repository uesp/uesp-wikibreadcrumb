...
class UespNamespace 
{
	...
	function __construct($ns_input=NULL, &$parser=NULL, $page_title=NULL )
	{
		...
		if ( is_null( $ns_input ) )
		{
			...
			if ($title->getArticleID() <= 0) $title = $wgTitle;
			...
			if (!is_object($title) || $title->getArticleID() < 0) $title = $parser->getTitle();
	...
	protected function get_entry($num)
	{
		if( array_key_exists( $num, $this->entries) && $this->entries[$num]!='') 
		{
			...
			if ($this->entries[$num]=='') return NULL;
	...
	{
		if( is_null( $this->data['ns_id'] = $this->get_entry(1) ) ) 	$this->data['ns_id'] = $this->data['ns_base'];
	...
	{
		if( is_null( $this->data['ns_parent'] = $this->get_entry(2) ) ) $this->data['ns_parent'] = $this->data['ns_base'];
	...
	{
		if( is_null( $this->data['ns_name'] = $this->get_entry(3) ) ) $this->data['ns_name'] = $this->data['ns_base'];
	...
	{
		if( is_null( $this->data['ns_mainpage'] = $this->get_entry(4) ) ) $this->data['ns_mainpage'] = $this->get('ns_full') . $this->get('ns_name');
	...
	{
		if( is_null( $this->data['ns_category'] = $this->get_entry(5) ) ) $this->data['ns_category'] = $this->data['ns_base'];
	...
	public static function find_nsobj( &$parser, $frame = NULL, $ns = NULL) 
	{
		...
		if (is_null($nschk))
		{
			...
			if (array_key_exists($titleid, self::$saved_by_title)) return self::$saved_by_title[$titleid];
		}
		...
		$obj = new UespNamespace($nschk, $parser);
		return $obj;
	}
	...
	public static function getRelatedNamespaces( $input_nsnum, &$extratext , &$nsprimary ) 
	{
		...
		foreach ($lines as $line)
		{
			...
			if( $entries[0]==$subjectspace ) $parent_ns = $entries[2];
		...
		if( is_null( $parent_ns )) 
		{
			...
			# in namespaces where search isn't broken, check this namespace too
				if ($nssubj != NS_MAIN && $nssubj != NS_SPECIAL && $nssubj != NS_USER && $nssubj != NS_MEDIAWIKI) $namespacelist[] = $nssubj;
				$nslist = $all_ns;
			}
		}
		else 
		...
		{
			if( !is_null( $parent_ns ) && $ns == $subjectspace ) continue;
			...
			if( $slash = stripos( $ns , ':' ))
			{
				...
				if( substr( $extratext, strlen($extratext)-1, 1 )=='/' ) $extratext = substr( $extratext, 0, strlen($extratext)-1 );
			}
			else 
			{
